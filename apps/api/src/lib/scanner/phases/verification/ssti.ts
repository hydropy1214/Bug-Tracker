...
export async function verifySsti(
  ...
  const candidate = candidates.find(
    (finding) => /SSTI/i.test(finding.title) && finding.verification === 'suspected',
  );
  ...
  if (!endpoint || !parameter) {
    await log(
      '[Phase 24] SSTI verification skipped — no stable endpoint and parameter were captured.',
    );
    return [];
  }
  ...
  await log(
    `[Phase 24] SSTI canary VERIFIED at ${endpoint} (${parameter}); no code execution attempted.`,
  );
  return [
    createFinding({
      ...
      limitations:
        'Template evaluation was confirmed with a string-only expression. No operating-system command, file access, or network access was attempted.',
      ...
      remediation:
        'Do not render untrusted input as a template. Use a sandboxed engine, strict escaping, and a least-privilege service account.',
    }),
  ];
}
