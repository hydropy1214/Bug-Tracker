...
export async function phase24Verification(
  ...
  if (!policy.allowVerification || !activeProbesAllowed()) {
    await log(
      '[Phase 24] Verification not run because the profile is not authorized or a WAF/rate-limit challenge was detected.',
    );
    return [];
  }
  ...
  await log(
    `[Phase 24] Verification complete — ${findings.length} bounded confirmation finding(s); allowance capped at ${policy.verificationRequestBudget} request(s).`,
  );
  return findings;
}
