...
export async function verifySqlInjection(
  ...
  const candidate = candidates.find((finding) =>
  ...
  if (!endpoint || !parameter) {
    await log("[Phase 24] SQLi verification skipped — no stable endpoint and parameter were captured.");
    return findings;
  }
  ...
  findings.push(createFinding({
    ...
    limitations: "Only a fixed canary was requested. No schema discovery, data extraction, authentication bypass, or modification was attempted.",
    ...
    remediation: "Use parameterised queries or prepared statements for every database operation. Do not concatenate request values into SQL.",
  }));
