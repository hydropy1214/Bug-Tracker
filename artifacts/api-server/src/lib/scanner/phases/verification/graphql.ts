...
export async function verifyGraphql(
  ...
  const endpoints = candidate
    ? [endpointFromFinding(candidate.title, candidate.evidence, target.url) ?? `${target.url.replace(/\/$/, "")}/graphql`]
    : [
        `${target.url.replace(/\/$/, "")}/graphql`,
        `${target.url.replace(/\/$/, "")}/api/graphql`,
      ];
  ...
  for (const endpoint of endpoints) {
    ...
    await log(`[Phase 24] GraphQL introspection VERIFIED at ${endpoint}; no sensitive fields queried.`);
    ...
    negativeTests: "Only the schema root name was requested; no mutations or application data were queried.",
    limitations: "This check confirms schema introspection only. It does not test authorization, query depth, or sensitive fields.",
    description: "The GraphQL endpoint accepted a minimal read-only introspection query and returned schema metadata.",
    cvss: 5.3,
    cve: null,
      evidence: `REQUEST: POST ${endpoint}\nQUERY: {__schema{queryType{name}}}\nRESPONSE: HTTP ${response.status}\nSCHEMA MARKER: __schema present`,
    remediation: "Disable introspection in production where it is not required, or restrict it to trusted tooling and authenticated development environments.",
    })];
  }
  return [];
}