...
export interface RealFinding {
  ...
  /** True only when a bounded verification phase produced direct evidence. */
  verified?: boolean;
...
/** Reserve one request from the bounded safe-verification allowance. */
...
async function checkWafAndBypass(target: Target, onLog: LogFn, allowBypass = false): Promise<{ findings: RealFinding[]; wafName: string | null }> {
  const findings: RealFinding[] = [];
  ...
  if (detectedWaf) {
    ...
    findings.push({
      ...
      cvss: 0, cve: null,
      description: `A Web Application Firewall (${detectedWaf}) is in front of this target. No bypass or evasion probes were attempted.`,
    ...
    if (!allowBypass) {
      await onLog(`[${ts()}] Passive WAF detection complete — bypass and origin discovery disabled by policy.`);
...
export async function scanTarget(
  ...
  // ── Phase 12: Safe web response checks ────────────────────────────────────
  await onLog(`[${ts()}] [Phase 12] Safe web checks — exploit payloads and auth manipulation disabled.`);
  await onLog(`[${ts()}] [Phase 12] SQLi, XSS, NoSQL, command execution, redirect, and auth-bypass probes are deferred to bounded checks only.`);

  // ── Phase 13: API surface discovery ──────────────────────────────────────
  await onLog(`[${ts()}] [Phase 13] API surface discovery — active schema queries deferred to bounded Phase 24 verification.`);
  ...
  // ── Phase 16–22: Restricted active modules ────────────────────────────────
  await onLog(`[${ts()}] [Phase 16–22] Restricted modules disabled: traversal/file reads, JWT forgery, IDOR escalation, request smuggling, exploit callbacks, brute-force, and advanced injection.`);

  // ── Phase 23: CVE lookup ───────────────────────────────────────────────────
  await onLog(`[${ts()}] [Phase 23] CVE database lookup (NVD) for detected technology versions...`);
  const { lookupCvesForTechs } = await import("./vuln-probes");
  add(await lookupCvesForTechs(techs, onLog));

  // ── Phase 24: Safe vulnerability verification ──────────────────────────────
  if (policy.allowVerification) {
    await onLog(`[${ts()}] [Phase 24] Safe vulnerability verification — bounded canaries, no credential attacks or file reads...`);
    const { phase24Verification } = await import("./scanner/phases/verification/phase-24-verification");
    add(await phase24Verification(target, policy, all.slice(), onLog));
  } else {
    await onLog(`[${ts()}] [Phase 24] Safe vulnerability verification skipped — profile does not permit active verification.`);
  }

  await onLog(`[${ts()}] Restricted exploit phases remain disabled; no credentials, tokens, sessions, files, metadata, internal hosts, or accounts were accessed.`);
  ...
  const top3 = reportable
    ...
    .join("\n");
      ...
      const verifiedCritical = verified.filter((f) => f.severity === "critical").length;
      const verifiedHigh = verified.filter((f) => f.severity === "high").length;
      await onLog(`[${ts()}] Verified vulnerabilities: ${verifiedCritical} critical, ${verifiedHigh} high (${verified.length} total)`);
      if (top3) {
